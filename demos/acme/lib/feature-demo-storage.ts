...
export function featureDemoStorageId(demoId: string, uid?: string | null): string {
  return uid ? `${demoId}:${uid}` : demoId;
}
...
export function clearFeatureDemoSessionDismissed(id: string, storage = defaultSession()): void {
  if (!storage) return;
  try {
