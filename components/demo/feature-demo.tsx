 ...
 * Tailwind port of the OTW / Eileen feature-demo pattern.
 */
export function FeatureDemo(props: FeatureDemoProps) {
  ...
  let popLeft: number;
  if (padded) {
    const spaceRight = viewport.w - padded.right - 16;
    const spaceLeft = padded.left - 16;
    const tall = padded.height > viewport.h * 0.38;
    if (tall && spaceRight >= 240) {
      popLeft = padded.right + GAP;
      popTop = Math.min(Math.max(16, padded.top), maxTop);
    } else if (tall && spaceLeft >= 240) {
      popLeft = padded.left - GAP - Math.min(popWidth, spaceLeft - GAP);
      popTop = Math.min(Math.max(16, padded.top), maxTop);
    } else if (tall) {
      popLeft = Math.max(16, viewport.w - popWidth - 16);
      popTop = maxTop;
    } else {
  ...
  const mask = 'pointer-events-auto bg-zinc-950/55';
  ...
  return createPortal(
    <div className="pointer-events-none fixed inset-0 z-[80]">
      ...
      <div
        ...
        className="demo-pop pointer-events-auto fixed rounded-2xl border border-white/10 bg-zinc-950/95 p-5 text-zinc-100 shadow-2xl shadow-black/60 backdrop-blur-xl"
